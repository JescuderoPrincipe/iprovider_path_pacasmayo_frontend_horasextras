sap.ui.define([
	"AgenGestHuma/agengesthuma/controller/BaseController",
	...
	FilterGroupItem,
	FilterBar, MessageBox, ComboBox, Item, CheckBox, ListItem, SuggestionItem) {
	"use strict";
	var oTable = new Table();
	var tabla = "ZTPT_AGENTE_GH";
	return BaseController.extend("AgenGestHuma.agengesthuma.controller.Home", {
		onInit: function () {
		...
		_attachMatched: function (oEvent) {
			...
			if (localStorage.fecha1) {
				var diferencia = new Date() - new Date(localStorage.fecha1);
				diferencia = diferencia / 1000;

				if (diferencia > 3) {
					location.reload();
					localStorage.removeItem("fecha1");
				}

			} else {
				localStorage.setItem("fecha1", fecha);
			}
			...
			oModel2.read("/parametersSet", {
				...
				success: function (result) {
					...
					thes.createTeable(result.results);
			...
			//this.onMatchCode("USER_ADDRP", "BNAME", "NAME_TEXT", "bsBNAME");
		...
		onBusquedaSensitiva: function (valor, filter1, filter2, objeto, modelo, path1, input2) {
			...
			oModel.read("/singleMatchCodeSet", {
				filters: ofilters,
				success: function (result) {
					//var jsonModel = new sap.ui.model.json.JSONModel(JSON.parse(result.results[0].et_data));
					//sap.input2.setModel(jsonModel);
					sap.input2.destroySuggestionItems();
					for (var j = 0; j < JSON.parse(result.results[0].et_data).length; j++) {
						/*sap.input2.addSuggestionItem(new sap.ui.core.Item({
							//key: oModel.getData()[j].VALUE,
					...
					//modelo.setProperty(path1 + "/bs" + objeto, JSON.parse(result.results[0].et_data));
					modelo.refresh();
					sap.ui.core.BusyIndicator.hide();
				},
				error: function (err) {
		...
		onTemplate: function (result, i) {
			...
			case "PERSA":
				...
				/*var comboBox = new ComboBox({
					...
					editable: "{= ${NEW} === true || ${" + result[i][Object.keys(result[i])[2]] + "} === '' ? true : false }",
					selectedKey: "{" + result[i][Object.keys(result[i])[2]] + "}"
				});
				...
				comboBox.addCustomData(oDataTemplate);

				var item = new ListItem({
					key: "{bs" + result[i][Object.keys(result[i])[2]] + ">VALUE}",
					text: "{bs" + result[i][Object.keys(result[i])[2]] + ">TEXTO}",
					additionalText: "{bs" + result[i][Object.keys(result[i])[2]] + ">VALUE}"
				});
				comboBox.bindItems({
					path: "bs" + result[i][Object.keys(result[i])[2]] + ">/",
					template: item
				});

				//comboBox.bindItems("/bs" + result[i][Object.keys(result[i])[2]] + ">/", item);
				...
				var input = new Input({
					...
					editable: "{= ${ACCION} === 'N' || " + result[i][Object.keys(result[i])[3]] + " === false || ${" + result[i][Object.keys(result[
						i])[
						2]] + "} === '' || ${" + result[i][Object.keys(result[i])[
						2]] + "} === " + 0 + " ? true : false }"
				});
		...
		onLiveChange: function (oEvent) {
			...
			case "BUKRS":
				...
				break;
			case "PERSA":
				sap.this.onBusquedaSensitiva(valor, oEvent.getSource().getModel().getProperty(path).BUKRS, "", "PERSA_GH", this.getModel(), path,
					this);
				break;
			case "BNAME":
