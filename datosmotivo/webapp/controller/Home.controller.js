sap.ui.define([
	"DatosMotivo/datosmotivo/controller/BaseController",
	...
	var tabla = "ZTPT_MOTIVO";
	return BaseController.extend("DatosMotivo.datosmotivo.controller.Home", {
		...
		_attachMatched: function () {
			...
			if (localStorage.fecha3) {
				var diferencia = new Date() - new Date(localStorage.fecha3);
				diferencia = diferencia / 1000;

				if (diferencia > 3) {
					location.reload();
					localStorage.removeItem("fecha3");
				}

			} else {
				localStorage.setItem("fecha3", fecha);
			}
		...
		onTemplate: function (result, i) {
			...
			case "PERSA":
				...
				var input = new Input({
					...
					editable: "{= ${ACCION} === 'N' || " + result[i][Object.keys(result[i])[3]] + " === false || ${" + result[i][Object.keys(result[i])[
