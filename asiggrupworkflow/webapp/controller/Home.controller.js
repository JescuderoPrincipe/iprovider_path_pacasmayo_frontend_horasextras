sap.ui.define([
	"AsigGrupWorkFlow/asiggrupworkflow/controller/BaseController",
	...
	var tabla = "ZTPT_ASG_GRP_WF";

	return BaseController.extend("AsigGrupWorkFlow.asiggrupworkflow.controller.Home", {
		onInit: function () {
		...
		_attachMatched: function () {
			...
			if (localStorage.fecha2) {
				var diferencia = new Date() - new Date(localStorage.fecha2);
				diferencia = diferencia / 1000;

				if (diferencia > 3) {
					location.reload();
					localStorage.removeItem("fecha2");
				}

			} else {
				localStorage.setItem("fecha2", fecha);
			}
			...
			//this.onMatchCode("PA0002", "PERNR", "CNAME", "bsPERNR");
		},
		...
		onTemplate: function (result, i) {
			...
			case "PERSA":
				/*var comboBox = new ComboBox({
					...
					editable: "{= ${NEW} === true || ${" + result[i][Object.keys(result[i])[2]] + "} === '' || ${" + result[i][Object.keys(result[i])[
						2]] + "} === " + 0 + " ? true : false }",
